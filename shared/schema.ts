...
export interface GoGenerateFlashcardsRequest {
  user_id: string;             // UUID esperado pelo backend
  prompt: string;              // Tópico para geração (equivalente ao 'topic')
  difficulty?: string;         // Nível de dificuldade para os flashcards
}
...
export function adaptFrontendToGoRequest(
  ...
  userId: string = "00000000-0000-0000-0000-000000000000" // UUID padrão para desenvolvimento
): GoGenerateFlashcardsRequest {
  return {
    user_id: userId,
    prompt: frontendRequest.topic,
    difficulty: frontendRequest.difficulty // Enviamos a dificuldade como campo separado
  };
}
