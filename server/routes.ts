...
function proxyRequest(req: Request, res: Response, targetUrl: string) {
  ...
  const options = {
    ...
    headers: {
      ...req.headers,
      host: url.host, // Substitui o host para o backend Go
    }
  };
  ...
  log(`Proxying ${req.method} request to ${targetUrl}`, "proxy");
  ...
  const proxyReq = httpClient.request(options, (proxyRes) => {
    ...
    proxyRes.on('end', () => {
      try {
        ...
        log(`Proxy response: ${JSON.stringify(parsedData, null, 2)}`, "proxy");
      } catch (e) {
        // Se não for JSON, apenas loga a resposta como texto
        log(`Proxy response: ${responseData}`, "proxy");
      }
  ...
  proxyReq.on('error', (err) => {
    log(`Erro no proxy para ${targetUrl}: ${err.message}`, "proxy");
  ...
  app.post('/api/flashcards/generate', (req, res) => {
    try {
      ...
      if (useGoBackend) {
        ...
        log(`Redirecionando para backend Go: POST ${targetUrl}`, "proxy");
        return proxyRequest(req, res, targetUrl);
      }
      
      // Implementação de fallback (dados simulados)
      log("Usando implementação de fallback para geração de flashcards", "proxy");
      res.status(200).json({
